...
function [Best_score, Best_pos, curve] = ECO2024(N, Max_iter, lb, ub, dim, fobj)
...
for i = 1:Max_iter
    elapsed_time = toc
    % if rem(i, 10) == 0
    %     display(['At iteration ', num2str(i), ' the fitness is ', num2str(curve(i-1)),' the code has run ', num2str(elapsed_time)]);
    % end
    R1 = rand(1);
    R2 = rand(1);
